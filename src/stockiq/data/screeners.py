...
ETF_UNIVERSE: list[dict] = [
    # Retail Favorites — most-watched by retail traders
...
@st.cache_data(ttl=CACHE_TTL["fetch_etf_scan"], show_spinner=False)
def fetch_etf_scan(categories: tuple[str, ...] | None = None) -> pd.DataFrame:
    ...
    Returns one row per ETF with price, returns, RSI, MA signal, and an ETF Score.
    """
    etfs = ETF_UNIVERSE
    if categories:
    ...
    # Benchmark: SPY 1M return for relative strength
    spy_ret_1m = 0.0
    try:
        if "SPY" in tickers:
    ...
    for ticker in tickers:
        try:
            ...
            # Volume surge: 5-day avg vs 20-day avg
            avg_vol_5  = float(df["Volume"].tail(5).mean())
            avg_vol_20 = float(df["Volume"].tail(20).mean())
            ...
            # ETF Score: momentum + RSI entry + volume
            score = 50.0
            if ret_1m is not None:
                score += min(ret_1m * 1.5, 15)          # +15 for strong 1M momentum
            if vs_spy is not None:
                score += min(vs_spy * 1.0, 10)          # +10 for outperforming SPY
            if rsi <= 40:
